...
1. Write an SQL query to find subscribers who have signed up for at least two different plans. Display the customer name, phone number, and the number of plans they have signed up for.
...
3. Write an SQL query to find subscribers who do not have any active signup (no signup record with `endDate = NULL`). Display the phone number and customer name.
...
1. **Add an `isCall` field to the `USES` table.**
